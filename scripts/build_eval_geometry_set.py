#!/usr/bin/env python3
"""Build frozen geometry eval set from data/public.jsonl → data/eval/geometry_dev.jsonl."""
...
DEFAULT_GEOMETRY_JSONL = EVAL_DIR / "geometry_dev.jsonl"
...
def build_eval_geometry_set(
    ...
    geometry_meta = {
        ...
        "jsonl": str(geometry_jsonl.relative_to(REPO)),
        "strata": dict(strata),
    }
    ...
    manifest.setdefault("watch", {})["geometry"] = geometry_meta
    ...
    return {"geometry": geometry_meta, "public_n": len(public_rows)}


def main() -> None:
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--public", type=Path, default=DEFAULT_PUBLIC, help="Labeled public JSONL")
    p.add_argument("--out", type=Path, default=DEFAULT_GEOMETRY_JSONL, help="Output geometry dev JSONL")
    p.add_argument("--manifest", type=Path, default=DEFAULT_MANIFEST, help="Watch manifest to update")
    p.add_argument("--force", action="store_true", help="Overwrite existing geometry dev file")
    args = p.parse_args()
    ...
    result = build_eval_geometry_set(args.public, args.out, args.manifest)
    geo = result["geometry"]
    print(f"Wrote {geo['n']} rows to {args.out}")
    print(f"  Public total: {result['public_n']}   strata={geo['strata']}")
