...
| [dev-008](dev-008-multi-blank-16k.md) | holdout_10p | 112 | **65.18%** | 78.38% | 58.67% | Same prompt stack, **no LoRA** |
...
OpenR1 1k × 1 epoch **does not show a clear holdout lift** over the multi_blank + 16k inference stack: overall **64.44%** sits ~**0.7 pp below** dev-008 and ~at the **dev-007 + multi_blank projection**. Training looks **healthy** (no format collapse, length up, MCQ emission strong). Target weak slices (**Q4 long**, **multi-blank ≥3**) remain at **40%**.

**Recommended next steps** (per pipeline):

1. **A/B:** base model vs LoRA on **same** `holdout_20p` with identical prompts (confirms flat is SFT-specific, not slice noise).
2. If still flat: **sft-003** (s1K-1.1) or re-run with **2 epochs** / mid-training checkpoint (this eval used `final_adapter` after **1 epoch only**).
3. Optional: **SC K=5** on this adapter via `holdout_10p` (cheap check per pipeline modest-win path).
